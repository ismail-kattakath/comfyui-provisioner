...
#
# On cold boot, /etc/portal.yaml is generated by supervisord's first
# caddy_config_manager.py run — which can race the env-var propagation
# and end up with a stale/minimal config (only "Instance" instead of
# all six apps). The result: comfyui's supervisor script greps for
# "ComfyUI" in /etc/portal.yaml, doesn't find it, and skips startup
# with "Skipping comfyui startup (not in /etc/portal.yaml)". The Open
# button then loads forever because the Portal can't reach ComfyUI.
#
# The official docs pattern is to rm /etc/portal.yaml from inside the
# PROVISIONING_SCRIPT, then supervisorctl restart so the regenerator
# runs again — by which point our PORTAL_CONFIG env var is reliably in
# place from /etc/environment.
if [ -n "${PORTAL_CONFIG:-}" ]; then
  echo "[onstart] PORTAL_CONFIG present -- removing /etc/portal.yaml so it regenerates"
  rm -f /etc/portal.yaml /etc/Caddyfile
  if command -v supervisorctl >/dev/null 2>&1; then
    # Restart the services that read PORTAL_CONFIG (instance_portal generates
    # portal.yaml; caddy reads it; comfyui greps it to decide whether to start)
    supervisorctl restart instance_portal caddy comfyui 2>&1 | sed 's/^/[onstart] /'
  fi
else
  echo "[onstart] WARNING: PORTAL_CONFIG not set -- 'Open' button may 404 and comfyui may skip startup."
  echo "[onstart]   Pass PORTAL_CONFIG via --env at instance create (see providers/vastai/README.md)."
fi
